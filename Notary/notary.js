...
function loadBook(filename, displayName) {
  ...
  // creating a serve request to load a book
  //  we have 0 1 2 3 & 4 so 0 is means that it

  // is unset and 1 would mean that the document or the file is now open and 2

  // means that it's received headers or it's got information about the request 3 means it's actually loading the the file

  // back and 4 means I am done and so when we get to ready state 4 that's when we

  // want to do some things that means that the load from the file is actually complete I also want to check in another

  // status and I want to check the HTTP status which is just status and I want

  // to make sure that this is successful so hxr status equals 200 which means okay

  // this this transaction is okay or this request is okay and if and so we're looking to see is it

  // complete and was it an okay transaction and when those are happening and when that's done that means that we can load

  // our variable and we can say give me the response text all right actually what it
